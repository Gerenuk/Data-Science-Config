...
unquoted_chars = set(string.ascii_letters) | set(string.digits) | set(" äöüÄÖÜß!?$%&/=`'+*~;_.-<>|@€")
...
def _has_short_repr(obj):  # these values can be printed on one line without breaks since they are small in repr
    return (isinstance(obj, (int, float, str, datetime.datetime, datetime.date)) or
            obj is None or
            (isinstance(obj, (list, dict, set, frozenset)) and len(
...
def _quote_text(text):
    ...
    if ({text[0], text[-1]} & quoted_boundary_chars or
        ...
        if string_display_quote_char not in text:
            ...
            return (string_display_quote_char +
                    text_repr.replace(string_display_quote_char, string_display_quote_char * 2) +
                    string_display_quote_char)
    else:
        return text_repr


def ipy_prettyprint_iter(obj, printer, is_cycle,
                         opentext, closetext,
                         maxunnested=3,
                         max_tail_length=3,
                         empty_iter=None,
    ...
    elif (len(obj) <= maxunnested and all(_has_short_repr(o) for o in obj)) or len(obj) == 1:
...
def ipy_prettyprint_dict(obj, printer, is_cycle,
    ...
    elif (len(obj) <= maxunnested and all(_has_short_repr(k) and _has_short_repr(v) for k, v in obj.items())) or len(
            obj) == 1:
        printer.text(opentext + " ")
            ...
            if len(item_cnt_list) > printer.max_seq_length:
                if len(item_cnt_list) > printer.max_seq_length:
                    if not isinstance(item_cnt_list, list):
                        printer.break_()
                        printer.text(ellipsis)
                    elif len(item_cnt_list) <= printer.max_seq_length + max_tail_length + 1:
...
def ipy_prettyprint_str(obj, printer, is_cycle, maxstrlen=200):
    if is_cycle:
        ...
        printer.text(_quote_text(obj[:maxstrlen // 2] + ellipsis + obj[-maxstrlen // 2:]))
    else:
        try:
            float(obj)
            printer.text(string_display_quote_char + str(
                obj) + string_display_quote_char)  # to disambiguate floats which are actually str
            # note that this does not catch something like IPs
        except ValueError:
            printer.text(_quote_text(obj))  # this is the default printer


def ipy_prettyprint_none(obj, printer, is_cycle):  # not used
    # assert obj is None
    printer.text("<None>")
...
try:  # Section for Text Printer
    ...
    # text_printer.for_type("builtins.NoneType", ipy_prettyprint_none)   # doesn't work
    text_printer.max_seq_length = max_seq_length

    try:
        import pandas as pd

        text_printer.for_type(pd.Timestamp, partial(ipy_prettyprint_datetime, prefix="pdTs"))
    except ImportError:
        pass
...
try:  # Section for HTML printer
    ...
    try:
        ...
        def ipy_html_dataframe(df, maxrows=max_seq_length):   # TODO: use more powerful method from below pandas?
    ...
    try:
        ...
        def ipy_html_pandasdataframe(df):
            ...
            colnames = [(str(colname) + (" \u246b" if _is_numpy_number(coltype) else "")) for colname, coltype
                        in zip(df.columns, df.dtypes)]

            if len(colnames) >= MAX_NUM_PD_COLNAMES:
                colnames = colnames[:MAX_NUM_PD_COLNAMES // 2] + [ellipsis] + colnames[-MAX_NUM_PD_COLNAMES // 2:]

            return "{} rows<br>{} cols: {}<br>{}".format(format_int(num_rows), num_cols, html.escape(", ".join(colnames)),
                                                         df._repr_html_())
        ...
        def ipy_html_pandasseries(s):
            length = len(s)
            return "{} elements of {}<br>{}".format(length, s.dtype, pd.DataFrame(s)._repr_html_())
